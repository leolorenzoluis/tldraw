...
const tldrawStateGlobalKey = Symbol.for('__@abc.xyz/state__')
const tldrawStateGlobal = globalThis as { [tldrawStateGlobalKey]?: true }

if (tldrawStateGlobal[tldrawStateGlobalKey]) {
	console.error(
		'Multiple versions of @abc.xyz/state detected. This will cause unexpected behavior. Please add "resolutions" (yarn/pnpm) or "overrides" (npm) in your package.json to ensure only one version of @abc.xyz/state is loaded.'
	)
} else {
